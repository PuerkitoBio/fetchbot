...
(https://github.com/temoto/robotstxt-go). It also integrates the `iq_slice.go`
file from the iq package (https://github.com/kylelemons/iq).
...
The following example shows how to create and start a Fetcher, one way to send commands,
and how to stop the fetcher once all commands have been handled.
...
fetch, and which HTTP method (verb) to use (i.e. "GET", "HEAD", ...).

A call to Fetcher.Start() returns the Queue associated with this Fetcher. This is the
thread-safe object that can be used to send more commands, or to stop the crawler.
...
knows which Command initiated this call, and has a handle on the Queue.

A Handler is similar to the net/http Handler, and middleware-style combinations can
be built on top of it. In fact, there is even a HandlerFunc type so that simple functions
with the right signature can be used as Handlers (like net/http.HandlerFunc), and there
is also a multiplexer Mux that can be used to dispatch calls based on some criteria.

The Fetcher recognizes some interfaces other than Command, for more advanced needs.
If the Command implements the BasicAuthProvider interface, a Basic Authentication
header will be put in place with the given credentials to fetch the URL.
...
ReaderProvider interface is used. It sets the body of the request (eg. for a "POST")
...
the duration that a worker goroutine can wait without receiving new commands to
fetch. If the idle time-to-live is reached, the worker goroutine is stopped and its
resources are released. This can be especially useful for long-running crawlers.
